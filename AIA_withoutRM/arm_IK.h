...
class IK
{
  ...
  public:
    ...
    void beginFK(int X, int Y, int Z)
    {
      ...
      // map to 0–255
      int pwmX = map(absX, 0, 11, 0, 255);
      int pwmZ = map(absZ, 0, 11, 0, 255);
      ...
      if (X == 0 && Z == 0) {
          ...
          // --- X only → both motors same speed (pure link1) ---
          bool dir = (X > 0) ? LOW : HIGH;
          digitalWrite(dir_link1, dir);
          digitalWrite(dir_link2, dir);
          ledcWrite(L1channel, pwmX);
          ledcWrite(L2channel, pwmX);
      }
      else if (X == 0 && Z != 0) {
          // --- Z only → only link2 moves ---
          digitalWrite(dir_link1, LOW);
          ledcWrite(L1channel, 0);
          bool dir2 = (Z > 0) ? LOW : HIGH;
          digitalWrite(dir_link2, dir2);
          ledcWrite(L2channel, pwmZ);
      }
      else {
          // --- X + Z together (differential control) ---

          // scale Z full, X at half contribution when equal
          // Example: X=11, Z=11 → pwmX=128, pwmZ=255
          int maxVal = max(absX, absZ);
          int scaledX = (absX * 255) / (absX + absZ); // proportionally smaller
          int scaledZ = (absZ * 255) / (absX + absZ);

          // --- Link1 motor (X) ---
          bool dir1 = (X > 0) ? LOW : HIGH;
          digitalWrite(dir_link1, dir1);
          ledcWrite(L1channel, scaledX);

          // --- Link2 motor (Z) ---
          bool dir2 = (Z > 0) ? LOW : HIGH;
          digitalWrite(dir_link2, dir2);
          ledcWrite(L2channel, scaledZ);
      }
      if (Y < 0)
      {
