...
#define dir_swivel 16 //J8
#define pwm_swivel 33
#define dir_link1 32//J12

#define pwm_link1 14
#define dir_link2 13 //J10
#define pwm_link2 23
    ...
    if (Z_index != nullptr && *(Z_index + 1) == '0') {
      if (M_index && x_index && y_index && P_index && Q_index && A_index && S_index && J_index && D_index && E_index) {
        ...
        if (Mode == 0) {
          ...
          if (Z > 0) {
            int pw1 = Z * 255;
            digitalWrite(dir_link2, HIGH);  //link1
            ledcWrite(ledChannelLink1, pw1);
          } else if (Z < 0) {
            int pw1 = -Z * 255;
            digitalWrite(dir_link2, LOW);  //link1
            ledcWrite(ledChannelLink1, pw1);
          } else {
            digitalWrite(dir_link2, LOW);  //link1
          ...
          if (X > 0) {
            int pw2 = X * 255;
            digitalWrite(dir_link1, HIGH);  //link2
            ledcWrite(ledChannelLink2, pw2);
          } else if (X < 0) {
            int pw2 = -X * 255;
            digitalWrite(dir_link1, LOW);  //link2
            ledcWrite(ledChannelLink2, pw2);
          } else {
            digitalWrite(dir_link1, LOW);  //link2
           ...
           if (Y > 0) {
            int pw3 = Y * 255;
            digitalWrite(dir_swivel, HIGH);  //link2
            ledcWrite(ledChannelSwivel, pw3);
          } else if (Y < 0) {
            int pw3 = -Y * 255;
            digitalWrite(dir_swivel, LOW);  //link2
            ledcWrite(ledChannelSwivel, pw3);
          } else {
            digitalWrite(dir_swivel, LOW);  //link2
